...
def pretrain(args, logger, device='cuda'):
    ...
    for model_id in range(args.pretrain_amount):
        ...
        init_path = os.path.join(args.pretrain_dir, f'{args.net_type}_premodel{model_id}_epoch0.pth.tar')
        model = define_model(args, args.net_type, args.depth, nclass).to(device)
        ...
        for epoch in range(args.pretrain_epochs):
            ...
            logger("<Pretraining {:2d}-th model>...[Epoch {:2d}] Train acc: {:.1f} (loss: {:.3f}), Val acc: {:.1f}".format(model_id,
                                                                                                                          ...
                                                                                                                          top1_val))
            scheduler.step()
            trained_path = os.path.join(args.pretrain_dir, f'{args.net_type}_premodel{model_id}_epoch{epoch+1}.pth.tar')
