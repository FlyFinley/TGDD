...
def matchloss(img_real, img_syn, model):
    with torch.no_grad():
        logits_tg, feat_tg = model(img_real, return_features=True)
    logits, feat = model(img_syn, return_features=True)
    loss = torch.sum((feat.mean(0) - feat_tg.mean(0))**2)
    return loss

def consloss(img_real, img_syn, label_syn, trained_model):
    logits, feat = trained_model(img_syn, return_features=True)
    loss = F.cross_entropy(logits, label_syn)
    return loss,None,feat
...
def condense(args, logger, device='cuda'):
    ...
    logger(f"\n TGDD: Start condensing for {n_iter} iteration")

    best_convnet = -1
    for it in range(n_iter):
        model_init = define_model(args, args.net_type, args.depth, nclass).to(device)
        model_final = define_model(args, args.net_type, args.depth, nclass).to(device)
        slkt_model_id = random.randint(0, args.num_premodel - 1)
        slkt_epoch_id_start = random.randint(0, args.pretrain_epochs)
        end_epoch = min(slkt_epoch_id_start+args.epoch_interval-1,args.pretrain_epochs)
        slkt_epoch_id_final = random.randint(slkt_epoch_id_start, end_epoch)
        final_path = os.path.join(args.pretrain_dir, f'{args.net_type}_premodel{slkt_model_id}_epoch{slkt_epoch_id_final}.pth.tar')
        model_final.load_state_dict(torch.load(final_path))
        init_path = os.path.join(args.pretrain_dir,f'{args.net_type}_premodel{slkt_model_id}_epoch{slkt_epoch_id_start}.pth.tar')
        ...
        for c in range(nclass):
            ...
            loss_match = matchloss(img_aug[:n], img_aug[n:], model_init)
            loss_cons, feat_tg, feat = consloss(img_aug[:n], img_aug[n:], label_syn, model_final)
            loss = loss_match+args.alpha*loss_cons
            optim_img.zero_grad()
            loss.backward()
        ...
        # Logging
        if it % it_log == 0:
            logger(f"{utils.get_time()} (Iter {it:3d}) loss: {loss:.2f}")
            # scheduler.step()
        save_best = 0
        if (it + 1) in it_test:
